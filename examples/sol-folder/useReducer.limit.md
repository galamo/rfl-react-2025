...
export default function SettingsPage() {
  ...
  const expenseLimitOptions = [5, 10, 20, 50, 100];
  ...
  return (
    <div className={styles.container}>
      ...
      <div className={styles.settingSection}>
        <h2>Expense Table Limit</h2>
        <p className={styles.description}>
          Set the maximum number of expense items to display in the table.
        </p>

        <div className={styles.expenseLimitOptions}>
          {expenseLimitOptions.map((limit) => (
            <button
              key={limit}
              className={`${styles.limitButton} ${
                state.expenseLimit === limit ? styles.active : ""
              }`}
              onClick={() => handleExpenseLimitChange(limit)}
            >
              <div className={styles.buttonContent}>
                <span className={styles.buttonTitle}>{limit} items</span>
              </div>
        ...
        <div className={styles.currentSetting}>
          <strong>Current limit:</strong> {state.expenseLimit} items
        </div>
      </div>
    </div>
  );
